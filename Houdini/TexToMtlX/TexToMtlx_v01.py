#######################################################################################################################
## TexToMtlX Version 1.0 by Ciro Cardoso                                                                             ##
...
texturesTypes = ['diffuse', 'diff', 'albedo', 'alb', 'base', 'col', 'color', 'basecolor',
                 ...
                 'ao', 'ambient_occlusion', 'occlusion',
...
class mainWindowUI (QMainWindow):
    ...
    def __init__(self):
        ...
        ## set window size
        self.setWindowTitle("TexToMtlX Version 1.0")
...
class material: ## blueprint to create the materialX
    ...
    def myfunc(self):
        ...
        group = subnetContext.parmTemplateGroup()

        ## FOLDER MATERIALX
        folder = hou.FolderParmTemplate(self.mat+"folder1", "MaterialX Builder", folder_type=hou.folderType.Collapsible, default_value=0, ends_tab_group=False)
        folder.setTags({"group_type": "collapsible", "sidefx::shader_isparm": "0"})

        ##INHERIT FROM CLASS
        inheritCtrl = hou.IntParmTemplate(self.mat+"inherit_ctrl", "Inherit from Class", 1, default_value=([2]), min=0, max=10, min_is_strict=False, max_is_strict=False, look=hou.parmLook.Regular, naming_scheme=hou.parmNamingScheme.Base1, menu_items=(["0","1","2"]), menu_labels=(["Never","Always","Material Flag"]), icon_names=([]), item_generator_script="", item_generator_script_language=hou.scriptLanguage.Python, menu_type=hou.menuType.Normal, menu_use_token=False)
        folder.addParmTemplate(inheritCtrl)

        ## CLASS ARC
        classArc = hou.StringParmTemplate(self.mat+"shader_referencetype", "Class Arc", 1, default_value=(["n = hou.pwd()\nn_hasFlag = n.isMaterialFlagSet()\ni = n.evalParm('inherit_ctrl')\nr = 'none'\nif i == 1 or (n_hasFlag and i == 2):\n    r = 'inherit'\nreturn r"]), default_expression=(["n = hou.pwd()\nn_hasFlag = n.isMaterialFlagSet()\ni = n.evalParm('inherit_ctrl')\nr = 'none'\nif i == 1 or (n_hasFlag and i == 2):\n    r = 'inherit'\nreturn r"]), default_expression_language=([hou.scriptLanguage.Python]), naming_scheme=hou.parmNamingScheme.Base1, string_type=hou.stringParmType.Regular, menu_items=(["none","reference","inherit","specialize","represent"]), menu_labels=(["None","Reference","Inherit","Specialize","Represent"]), icon_names=([]), item_generator_script="", item_generator_script_language=hou.scriptLanguage.Python, menu_type=hou.menuType.Normal)
        classArc.setTags({"sidefx::shader_isparm": "0", "spare_category": "Shader"})
        folder.addParmTemplate(classArc)

        ## Class Prim Path
        classPrim = hou.StringParmTemplate(self.mat+"shader_baseprimpath", "Class Prim Path", 1, default_value=(["/__class_mtl__/`$OS`"]), naming_scheme=hou.parmNamingScheme.Base1, string_type=hou.stringParmType.Regular, menu_items=([]), menu_labels=([]), icon_names=([]), item_generator_script="", item_generator_script_language=hou.scriptLanguage.Python, menu_type=hou.menuType.Normal)
        classPrim.setTags({"script_action": "import lopshaderutils\nlopshaderutils.selectPrimFromInputOrFile(kwargs)", "script_action_help": "Select a primitive in the Scene Viewer or Scene Graph Tree pane.\nCtrl-click to select using the primitive picker dialog.", "script_action_icon": "BUTTONS_reselect", "sidefx::shader_isparm": "0", "sidefx::usdpathtype": "prim", "spare_category": "Shader"})
        folder.addParmTemplate(classPrim)

        ## SEPARADOR
        separator = hou.SeparatorParmTemplate(self.mat+"separator")
        folder.addParmTemplate(separator)

        ## TAB MENU
        tabMenu = hou.StringParmTemplate(self.mat+"tabmenumask", "Tab Menu Mask", 1, default_value=(["MaterialX parameter constant collect null genericshader subnet subnetconnector suboutput subinput"]), naming_scheme=hou.parmNamingScheme.Base1, string_type=hou.stringParmType.Regular, menu_items=([]), menu_labels=([]), icon_names=([]), item_generator_script="", item_generator_script_language=hou.scriptLanguage.Python, menu_type=hou.menuType.Normal)
        tabMenu.setTags({"spare_category": "Tab Menu"})
        folder.addParmTemplate(tabMenu)

        ## RENDER CONTEXT
        renderContext = hou.StringParmTemplate(self.mat+"shader_rendercontextname", "Render Context Name", 1, default_value=(["mtlx"]), naming_scheme=hou.parmNamingScheme.Base1, string_type=hou.stringParmType.Regular, menu_items=([]), menu_labels=([]), icon_names=([]), item_generator_script="", item_generator_script_language=hou.scriptLanguage.Python, menu_type=hou.menuType.Normal)
        renderContext.setTags({"sidefx::shader_isparm": "0", "spare_category": "Shader"})
        folder.addParmTemplate(renderContext)

        ## FORCE 
        forceTrans = hou.ToggleParmTemplate(self.mat+"shader_forcechildren", "Force Translation of Children", default_value=True)
        forceTrans.setTags({"sidefx::shader_isparm": "0", "spare_category": "Shader"})

        group.append(folder)
        subnetContext.setParmTemplateGroup(group)
        subnetContext.parm(self.mat+'shader_referencetype').set("inherit")
        ...
        for texture in getMat:
            for textureKey, textureValue in textureTypesSorted.items():
                if texture in textureValue:
                    ...
                    texMain = subnetContext.createNode("mtlximage", tex) # this creates a image for each the texture sets from listText - Alb, Disp, etc
